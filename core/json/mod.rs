...
use jsonb::{
    unescape_string, validate_jsonb, ElementType, Jsonb, JsonbHeader, ParseInfo, PathOperationMode,
...
fn looks_like_jsonb_blob(slice: &[u8]) -> bool {
    // SQLite has no 8-byte payload size encoding (header nibble 15).
    if slice.is_empty() || slice[0] >> 4 == 15 {
        return false;
    }
    let Ok((header, header_offset)) = JsonbHeader::from_slice(0, slice) else {
        return false;
    ...
    // validating fully and falling back to text when that fails.
    if payload_size <= 7 && matches!(slice[0], b'{' | b'[' | b'0'..=b'9') {
        return validate_jsonb(slice);
    }
    true
}
...
pub fn json_error_position(json: impl AsValueRef) -> crate::Result<Value> {
    match json.as_value_ref() {
        ...
        ValueRef::Blob(blob) => {
            // A fully valid JSONB blob has no error. Anything else is
            // read as text, like the other document arguments.
            if looks_like_jsonb_blob(blob) && validate_jsonb(blob) {
                return Ok(Value::from_i64(0));
            }
            match Jsonb::from_str(&String::from_utf8_lossy(blob)) {
                Ok(_) => Ok(Value::from_i64(0)),
                Err(JsonError::Message { location, .. }) => {
