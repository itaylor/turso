                    ...
                    match idx {
                        Some(idx) if *idx >= 0 => {
                            ...
                            if mode.allows_insert() && arr_pos == end_pos && count == *idx as usize
                            {
                                ...
                                self.data
                                    .splice(arr_pos..arr_pos, placeholder_bytes.iter().copied());
                                // Growing the payload can also grow the array
                                // header itself (at a size-format boundary such
                                // as 11 bytes), which shifts the contents right;
                                // the placeholder position and the parent delta
                                // must include that shift.
                                let new_header_size = self.write_element_header(
                                    value_idx,
                                    ElementType::ARRAY,
                                ...
                                let header_growth = new_header_size - value_header_size;
                                return Ok(JsonTraversalResult::with_array_index(
                                    value_idx,
                                    JsonLocationKind::ObjectProperty(key_idx),
                                    (placeholder_bytes.len() + header_growth) as isize,
                                    arr_pos + header_growth,
                                ));
                            }
...
#[inline]
/// Whether `data` is a fully valid JSONB document. Validates the
/// borrowed bytes directly, without copying them into a `Jsonb`.
pub(super) fn validate_jsonb(data: &[u8]) -> bool {
    validate_element(data, 0, data.len(), 0).is_ok()
}

fn validate_element(data: &[u8], start: usize, end: usize, depth: usize) -> Result<()> {
    if depth > MAX_JSON_DEPTH {
        bail_parse_error!("Too deep");
    }
    ...
    let (header, header_offset) = JsonbHeader::from_slice(start, data)?;
    ...
    match header.element_type() {
        ...
        ElementType::TEXT | ElementType::TEXTJ | ElementType::TEXT5 | ElementType::TEXTRAW => {
            ...
            std::str::from_utf8(payload)
                .map_err(|_| LimboError::ParseError("Invalid UTF-8 in text payload".to_string()))?;
            Ok(())
        }
        ElementType::ARRAY => {
            ...
            while pos < payload_end {
                ...
                let (elem_header, elem_header_size) = JsonbHeader::from_slice(pos, data)?;
                ...
                validate_element(data, pos, elem_end, depth + 1)?;
                pos = elem_end;
            }
            Ok(())
        }
